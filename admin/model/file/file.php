		...
		if($delete_id){
			foreach($delete_array_id as $delete_file_id){
				...
				if(isset($delete_array_name->row['file'])){
					if(file_exists(str_replace("system/storage/download", "files", DIR_DOWNLOAD) . $delete_array_name->row['file'])){
						unlink(str_replace("system/storage/download", "files", DIR_DOWNLOAD) . $delete_array_name->row['file']);
					}				
				}				
			}
		}
